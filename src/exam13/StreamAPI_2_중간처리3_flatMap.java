...
public class StreamAPI_2_중간처리3_flatMap {
	...
	public static void main(String[] args) {
		...
		/*
		 * map(Function<T,R>) - 리턴 타입 R (한 종류)
		 * 
		 * flatMap(Function<T,Stream>) - 리턴 타입이 스트림 (여러개)
		 * 역할 : 요소를 가공하여 복수개의 요소들로 구성된 새로운 스트림을 반환한다.
		 */
		
		List<String> list = Arrays.asList("hello world","happy new year");
		
		//실습1. 공백문자 기준으로 분리
		//익명클래스
		Function<String, Stream<? extends String>> mapper =
				new Function<String, Stream<? extends String>>() { //String을 상속받는 Stream
